...
export function DailyView({ dailyTasks, onAdd, onRemove, onEdit, completions, setCompletions }) {
    const [adding, setAdding] = useState(false);
    const [newTask, setNewTask] = useState({ task: "", time: "Opening", category: "Sanitation" });
    const [editId, setEditId] = useState(null);
    const [editVal, setEditVal] = useState({});

    const toggle = (id) => setCompletions({ ...completions, [id]: !completions[id] });
    const addTask = () => {
        ...
        <div style={{ maxWidth: "800px" }}>
            <Card style={{ marginBottom: "22px" }}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "10px" }}>
                    ...
                    <span style={{ fontSize: "14px", color: pct === 100 ? "#166534" : G, fontWeight: "700" }}>{done} / {dailyTasks.length} — {pct}%</span>
                </div>
            ...
            {TIMES.map(time => {
                const group = dailyTasks.filter(t => t.time === time);
                                ...
                                <div key={t.id} style={{ display: "flex", alignItems: "center", gap: "10px", padding: "11px 14px", marginBottom: "5px", background: isDone ? "#f0fdf4" : WH, border: isDone ? "1.5px solid #86efac" : "1.5px solid #ececec", borderRadius: "8px" }}>
                                    ...
                                    {isEdit ? (
                                        ...
                                        <>
                                            ...
                                            <button onClick={() => { setEditId(t.id); setEditVal({ task: t.task, category: t.category, time: t.time }); }} style={{ padding: "4px 10px", background: WH, border: "1.5px solid #ddd", borderRadius: "6px", color: "#555", fontSize: "11px", cursor: "pointer", fontWeight: "600" }}>Edit</button>
                                            <button onClick={() => onRemove(t.id)} style={{ padding: "4px 10px", background: "#fff5f5", border: "1.5px solid #fca5a5", borderRadius: "6px", color: "#991b1b", fontSize: "11px", cursor: "pointer", fontWeight: "600" }}>✕</button>
                                        </>
                                    )}
                                </div>
                            );
